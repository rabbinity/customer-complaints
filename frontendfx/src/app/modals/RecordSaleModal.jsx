...
const RecordSaleModal = ({ isOpen, onClose, onSaleRecorded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/A/sale`, {
        storeId,
        productId,
        quantitySold: Number(quantitySold),
      });
      setBackendResponse(response.data);
      onSaleRecorded && onSaleRecorded(response.data);
      setStoreId("");
      setProductId("");
      setQuantitySold("");
    } catch (err) {
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className="block text-gray-700 mb-1">Quantity Sold</label>
                <input
                  type="number"
                  className="w-full border border-gray-300 rounded-md px-3 py-2"
                  value={quantitySold}
